...
export default function Page() {
  ...
  return <main ref={page}>
    ...
    <section className="about section-shell scroll-section" id="about"><div className="eyebrow scroll-copy">01 · 关于我</div><div className="section-title-wrap"><p className="section-title-english scroll-title">ABOUT.</p></div><div className="about-grid"><TiltedCard className="portrait-frame scroll-card" rotateAmplitude={2.2}><img className="portrait-photo" src="/images/ren-guopeng-portrait.jpg" alt="任国鹏个人照片" /><span>REN GUOPENG / 3D HMI</span></TiltedCard><TiltedCard className="about-content" rotateAmplitude={1.4}><h2 className="scroll-subtitle">让每一帧动态<br />都服务于<span>理解与感受。</span></h2><p className="scroll-copy">我是一名专注于 3D HMI 与动态体验的设计师，拥有 7 年工业设计与智能座舱设计经验。通过清晰的视觉语言与精确的运动节奏，让复杂系统自然地被感知。</p><div className="pill-row scroll-copy"><span>上海 / 可远程</span><a href="mailto:1019431896@qq.com">1019431896@qq.com ↗</a><span>汽车 · 3D · 动效</span></div></TiltedCard></div><div className="stats">{[['07+', '年设计经验'], ['05+', '汽车项目经验'], ['01', '红点奖项目']].map(([value, label]) => <TiltedCard className="scroll-card" rotateAmplitude={2} scaleOnHover={1.025} key={label}><strong>{value}</strong><span>{label}</span></TiltedCard>)}</div></section>

    <section className="work scroll-section" id="work"><div className="section-shell work-heading"><div className="eyebrow scroll-copy">02 · 精选项目</div><div><div className="section-title-wrap"><h2 className="scroll-title">SELECTED<br /><span>WORK.</span></h2></div><p className="scroll-copy">从概念到量产，以三维设计与动效塑造下一代车载体验。</p></div></div><div className="project-grid section-shell">{projects.map((project) => <TiltedCard className="project-card scroll-card" rotateAmplitude={2.4} scaleOnHover={1.015} key={project.number}><Link className="project-link" href={`/projects/${project.slug}`} aria-label={`查看 ${project.name} 项目`}><div className="project-image">{project.previewVideo ? <video src={project.previewVideo} autoPlay muted loop playsInline preload="metadata" aria-label={`${project.name} 项目预览视频`} /> : <img src={project.image} alt={`${project.name} 项目展示`} />}</div><div className="project-info"><div><h3>{project.name}</h3><p>{project.label}</p></div><span className="project-open" aria-hidden="true">↗</span></div></Link></TiltedCard>)}</div></section>
